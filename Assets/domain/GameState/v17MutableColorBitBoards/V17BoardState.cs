...
public class V17BoardState : BoardStateInterface
{
  ...
  public V17BoardState()
  {
    ...
    whiteBitBoard = bitBoards[V17BoardState.WhitePawn]
      | bitBoards[V17BoardState.WhiteRook]
      | bitBoards[V17BoardState.WhiteKnight]
      | bitBoards[V17BoardState.WhiteBishop]
      | bitBoards[V17BoardState.WhiteQueen]
      | bitBoards[V17BoardState.WhiteKing];

    blackBitBoard = bitBoards[V17BoardState.BlackPawn]
      | bitBoards[V17BoardState.BlackRook]
      | bitBoards[V17BoardState.BlackKnight]
      | bitBoards[V17BoardState.BlackBishop]
      | bitBoards[V17BoardState.BlackQueen]
      | bitBoards[V17BoardState.BlackKing];
  }

  public V17BoardState(BoardStateInterface other)
  {
  ...
  public V17BoardState(bool whiteTurn, List<PiecePosition> piecePositions, CastleFlags castleFlags)
  {
  ...
  public BoardStatePlay PlayMove(Move move)
  {
    ...
    if (whiteTurn)
    {
      this.whiteBitBoard ^= sourceBitBoard | targetBitBoard;
      this.blackBitBoard &= ~targetBitBoard;
    }
    else
    {
      this.blackBitBoard ^= sourceBitBoard | targetBitBoard;
      this.whiteBitBoard &= ~targetBitBoard;
    }
    ...
    if (sourcePieceType.IsPawn() && move.source.col != move.target.col && killedPiece == null)
    {
      ...
      this.whiteBitBoard &= ~killedPieceBitBoard;
      this.blackBitBoard &= ~killedPieceBitBoard;
    }
    ...
    if (sourcePieceType.IsKing() && Math.Abs(move.target.col - move.source.col) == 2)
    {
      ...
      if (whiteTurn)
      {
        this.whiteBitBoard ^= rookSwap.bitBoard;
      }
      else
      {
        this.blackBitBoard ^= rookSwap.bitBoard;
      }
  ...
  public void UndoMove(ReversibleMove reversibleMove)
  {
    ...
    if (reversibleMove.killed != null)
    {
      ...
      if (!whiteTurn)
      {
        this.blackBitBoard |= killedBitBoard;
      }
      else
      {
        this.whiteBitBoard |= killedBitBoard;
      }
  ...
  public class Hashable : BoardStateInterface
  {
    ...
    public CastleFlags CastleFlags { get => castleFlags; }
    private CastleFlags castleFlags;
    public int EnPassantColumn { get => enPassantColumn; }
    private int enPassantColumn;

    public Hashable(V17BoardState boardState)
    {
    ...
    public V17BoardState UndoMove(ReversibleMove reversibleMove)
    {
      throw new Exception("Method not supported");
    }

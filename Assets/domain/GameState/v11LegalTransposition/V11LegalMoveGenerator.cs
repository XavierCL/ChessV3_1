...
public static class V11LegalMoveGenerator
{
  ...
  public static List<Move> GenerateLegalMoves(this V11GameState gameState)
  {
    ...
    if (legalCache == null) legalCache = new HashsetCache<V11BoardState, List<Move>>(99_991);
  ...
  private static List<Move> GenerateBoardLegalMoves(V11BoardState boardState)
  {
    var bitCount = boardState.allBitBoard.bitCount();
    if (bitCount <= 4)
    {
      // Two kings: draw
      if (bitCount < 3) return new List<Move>();

      var bishopSquare = 2;
      var bishopColor = 2;
      var piecePositions = boardState.piecePositions;
      for (var pieceIndex = 0; pieceIndex < piecePositions.Count; ++pieceIndex)
      {
        var piecePosition = piecePositions[pieceIndex];
        if (piecePosition.pieceType.IsKing()) continue;
        if (piecePosition.pieceType.IsKnight())
        {
          if (bitCount > 3) break;

          // Two kings and a knight, draw
          return new List<Move>();
        }
        if (!piecePosition.pieceType.IsBishop()) break;
        if (bishopColor == 2)
        {
          // Two kings and a bishop, draw
          if (bitCount == 3) return new List<Move>();

          bishopSquare = piecePosition.position.IsWhiteSquare() ? 1 : 0;
          bishopColor = piecePosition.pieceType.IsWhite() ? 1 : 0;
          continue;
        }

        if (bishopSquare == (piecePosition.position.IsWhiteSquare() ? 1 : 0) && bishopColor != (piecePosition.pieceType.IsWhite() ? 1 : 0))
        {
    ...
    var pseudoLegalMoves = GeneratePseudoLegalMoves(boardState, boardState.whiteTurn);
    ...
    for (var index = 0; index < pseudoLegalMoves.Count; ++index)
    {
      ...
      if (!CanKingDieAfterMove(boardState, move, boardState.whiteTurn))
      {
        legalMoves.Add(move);
      }
    }
  ...
  public static bool CanOwnKingDie(V11GameState gameState)
  {
    return CanKingDie(gameState.boardState, gameState.BoardState.whiteTurn);
  }

  private static bool CanKingDieAfterMove(V11BoardState boardState, Move ownMove, bool whiteKing)
  {
    var kingMightBeInCheckState = boardState.PlayMove(ownMove).boardState;
    return CanKingDie(kingMightBeInCheckState, whiteKing);
  }

  private static bool CanKingDie(V11BoardState boardState, bool whiteKing)
  {
    ...
    var jumps = V11Precomputed.knightJumps[kingPosition];
  ...
  private static List<Move> GeneratePseudoLegalMoves(V11BoardState boardState, bool white)
  {
  ...
  private static List<Move> GetPseudoLegalMoves(V11BoardState boardState, BoardPosition position, PieceType pieceType)
  {
    switch (pieceType)
    {
  ...
  private static List<Move> GetPseudoLegalPawnMoves(V11BoardState boardState, BoardPosition position, PieceType pieceType)
  {
  ...
  private static List<Move> GetPseudoLegalRookMoves(V11BoardState boardState, BoardPosition position, PieceType pieceType)
  {
  ...
  private static List<Move> GetPseudoLegalKnightMoves(V11BoardState boardState, BoardPosition position, PieceType pieceType)
  {
    var validMoves = new List<Move>(8);
    var jumps = V11Precomputed.knightJumps[position.index];
  ...
  private static List<Move> GetPseudoLegalBishopMoves(V11BoardState boardState, BoardPosition position, PieceType pieceType)
  {
  ...
  private static List<Move> GetPseudoLegalQueenMoves(V11BoardState boardState, BoardPosition position, PieceType pieceType)
  {
    return GetPseudoRayMoves(boardState, position, 1, 0, pieceType.IsWhite())
      .Concat(GetPseudoRayMoves(boardState, position, -1, 0, pieceType.IsWhite()))
      .Concat(GetPseudoRayMoves(boardState, position, 0, 1, pieceType.IsWhite()))
  ...
  private static List<Move> GetPseudoLegalKingMoves(V11BoardState boardState, BoardPosition position, PieceType pieceType)
  {
    var landings = new[]{
  ...
  private static List<Move> GetPseudoRayMoves(V11BoardState boardState, BoardPosition position, int colIncrement, int rowIncrement, bool isWhite)
  {
  ...
  {
    var values = V11Precomputed.bitBoardAtRayBranches[(colIncrement + 1) * 3 + rowIncrement + 1][position] & allPieces;

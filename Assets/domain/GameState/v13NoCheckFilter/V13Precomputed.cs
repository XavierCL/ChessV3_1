...
public static class V13Precomputed
{
  ...
  public readonly static ulong[][] pinRays = GeneratePinRays();
  ...
  private static ulong[][] GeneratePinRays()
  {
    var allPinRays = new ulong[64][];
    for (var sourcePosition = 0; sourcePosition < 64; ++sourcePosition)
    {
      var pinRays = new ulong[64];
      for (var targetPosition = 0; targetPosition < 64; ++targetPosition)
      {
        var colDistance = targetPosition.getCol() - sourcePosition.getCol();
        var rowDistance = targetPosition.getRow() - sourcePosition.getRow();

        if (Math.Abs(colDistance) != Math.Abs(rowDistance) && colDistance != 0 && rowDistance != 0) continue;
        if (colDistance == 0 && rowDistance == 0)
        {
          pinRays[targetPosition] = targetPosition.toBitBoard();
          continue;
        }

        var maxDistance = Math.Max(Math.Abs(colDistance), Math.Abs(rowDistance));
        var colIncrement = colDistance / maxDistance;
        var rowIncrement = rowDistance / maxDistance;

        var pinRay = targetPosition.toBitBoard();
        var currentPosition = sourcePosition;
        while (currentPosition != targetPosition)
        {
          pinRay |= currentPosition.toBitBoard();
          currentPosition = BoardPosition.fromColRow(currentPosition.getCol() + colIncrement, currentPosition.getRow() + rowIncrement);
        }

        pinRays[targetPosition] = pinRay;
      }

      allPinRays[sourcePosition] = pinRays;
    }

    Debug.Log(string.Join(", ", allPinRays.Select(pinRays => "new ulong[] { " + string.Join(", ", pinRays.Select(pinRay => "0x" + pinRay.ToString("X") + "ul")) + " } ")));

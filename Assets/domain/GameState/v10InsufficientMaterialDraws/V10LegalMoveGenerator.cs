...
public static class V10LegalMoveGenerator
{
  /// <summary>
  /// Doesn't take end game draws in consideration (three-fold nor 50 moves)
  /// </summary>
  public static List<Move> GenerateLegalMoves(this V10GameState gameState)
  {
    ...
    var bitCount = gameState.boardState.allBitBoard.bitCount();
    if (bitCount <= 4)
    {
      // Two kings: draw
      ...
      var bishopSquare = 2;
      var bishopColor = 2;
      var piecePositions = gameState.boardState.piecePositions;
  ...
  public static bool CanOwnKingDie(V10GameState gameState)
  {
    return CanKingDie(gameState.boardState, gameState.BoardState.whiteTurn);
  }

  private static bool CanKingDieAfterMove(V10BoardState boardState, Move ownMove, bool whiteKing)
  {
    var kingMightBeInCheckState = boardState.PlayMove(ownMove).boardState;
    return CanKingDie(kingMightBeInCheckState, whiteKing);
  }

  private static bool CanKingDie(V10BoardState boardState, bool whiteKing)
  {
    ...
    var jumps = V10Precomputed.knightJumps[kingPosition];
  ...
  private static List<Move> GeneratePseudoLegalMoves(V10BoardState boardState, bool white)
  {
  ...
  private static List<Move> GetPseudoLegalMoves(V10BoardState boardState, BoardPosition position, PieceType pieceType)
  {
    switch (pieceType)
    {
  ...
  private static List<Move> GetPseudoLegalPawnMoves(V10BoardState boardState, BoardPosition position, PieceType pieceType)
  {
  ...
  private static List<Move> GetPseudoLegalRookMoves(V10BoardState boardState, BoardPosition position, PieceType pieceType)
  {
  ...
  private static List<Move> GetPseudoLegalKnightMoves(V10BoardState boardState, BoardPosition position, PieceType pieceType)
  {
    var validMoves = new List<Move>(8);
    var jumps = V10Precomputed.knightJumps[position.index];
  ...
  private static List<Move> GetPseudoLegalBishopMoves(V10BoardState boardState, BoardPosition position, PieceType pieceType)
  {
  ...
  private static List<Move> GetPseudoLegalQueenMoves(V10BoardState boardState, BoardPosition position, PieceType pieceType)
  {
    return GetPseudoRayMoves(boardState, position, 1, 0, pieceType.IsWhite())
      .Concat(GetPseudoRayMoves(boardState, position, -1, 0, pieceType.IsWhite()))
      .Concat(GetPseudoRayMoves(boardState, position, 0, 1, pieceType.IsWhite()))
  ...
  private static List<Move> GetPseudoLegalKingMoves(V10BoardState boardState, BoardPosition position, PieceType pieceType)
  {
    var landings = new[]{
  ...
  private static List<Move> GetPseudoRayMoves(V10BoardState boardState, BoardPosition position, int colIncrement, int rowIncrement, bool isWhite)
  {
  ...
  {
    var values = V10Precomputed.bitBoardAtRayBranches[(colIncrement + 1) * 3 + rowIncrement + 1][position] & allPieces;

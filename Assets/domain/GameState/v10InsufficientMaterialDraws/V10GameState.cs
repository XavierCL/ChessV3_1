...
public class V10GameState : GameStateInterface
{
    ...
    public V10BoardState boardState;
    ...
    public Dictionary<V10BoardState, ushort> snapshots { get; }
    ...
    public V10GameState()
    {
        ...
        boardState = new V10BoardState();
        snapshots = new Dictionary<V10BoardState, ushort>();
    }

    public V10GameState(GameStateInterface gameState)
    {
        staleTurns = gameState.StaleTurns;
        history = new List<ReversibleMove>(gameState.history);
        boardState = new V10BoardState(gameState.BoardState);
        snapshots = gameState.Snapshots.ToDictionary(tuple => new V10BoardState(tuple.Key), tuple => tuple.Value);
    }

    public V10GameState(List<PiecePosition> piecePositions, bool whiteStarts, CastleFlags castling)
    {
        staleTurns = 0;
        history = new List<ReversibleMove>();
        boardState = new V10BoardState(
    ...
    public override GameEndState GetGameEndState()
    {
        ...
        var canOwnKingDie = V10LegalMoveGenerator.CanOwnKingDie(this);

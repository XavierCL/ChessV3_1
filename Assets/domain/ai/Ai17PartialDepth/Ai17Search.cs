public static class Ai17Search
{
  // Depths are decreasing. A depth of 1 means evaluation
  public static Ai17SearchResult Search(V17GameState gameState, int depth, Ai17SearchResult alpha, Ai17SearchResult beta, Ai17SearchResult.HyperParameters hyperParameters)
  {
    if (depth <= 1) return Ai17SearchExtension.Search(gameState, alpha, beta, hyperParameters);
    ...
    if (legalMoves.Count == 0)
    {
      ...
      {
        return new Ai17SearchResult(double.MaxValue, true, 1);
      }
      else if (endGameState == GameEndState.BlackWin)
      {
        return new Ai17SearchResult(double.MinValue, true, 1);
      }

      return new Ai17SearchResult(0, true, 1);
    }

    var bestSearchResult = beta;
    var allTerminalLeaves = true;
    var nodeCount = 1L;
    var sortedMoves = depth - 1 > hyperParameters.sortFromDepth ? Ai17Evaluate.SortMoves(legalMoves, gameState) : legalMoves;

...
public class Ai7 : MonoBehaviour, AiInterface
{
    ...
    public Task<Move> GetMove(GameStateInterface referenceGameState, TimeSpan remainingTime, TimeSpan increment)
    {
        ...
        if (ShowDebugInfo)
        {
            Debug.Log($"Ai7 Depth: {depth}, ratio: {lastCurrentMoveIndex}/{legalMoves.Count}, Elapsed: {timeManagement.GetElapsed().TotalSeconds}, Best moves: {bestIndicesEver.Count}, Evaluation: {bestValueEver}");
        }
